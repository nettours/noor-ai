...
export default function ChatPage() {
  ...
  const [me, setMe] = useState({ id: '', name: '', token: '' });
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      console.error(e);
      toast('تعذّر تحميل جهات الاتصال', 'error');
    } finally {
  ...
  useEffect(() => {
    ...
    socket.on('user:new', (newUser: Contact) => {
      setContacts(prev => {
        ...
        toast('👋 انضم ' + newUser.name + ' للتطبيق', 'info');
        return [...prev, newUser];
      });
    });
    ...
    // إشعار رسالة جديدة (في محادثة غير مفتوحة)
    socket.on('chat:notify', ({ message }: any) => {
      const sender = contacts.find(c => c.id === message.senderId);
      toast('💬 ' + (sender?.name || 'رسالة جديدة'));
    });

    return () => { socket.disconnect(); };
  }, [me.id, me.name, active]);
  ...
  useEffect(() => {
    ...
    return () => { socketRef.current?.emit('chat:leave', { conversationId: convId }); };
  }, [active, me.id]);
  ...
  const startTyping = () => {
    if (!active || !socketRef.current) return;
    socketRef.current.emit('chat:typing', { conversationId: getConvId(me.id, active.id), isTyping: true });
    clearTimeout(typingTimeoutRef.current);
    typingTimeoutRef.current = setTimeout(stopTyping, 2000);
  };
  const stopTyping = () => {
    if (!active || !socketRef.current) return;
    socketRef.current.emit('chat:typing', { conversationId: getConvId(me.id, active.id), isTyping: false });
    clearTimeout(typingTimeoutRef.current);
  };
  ...
  const handleFile = (file: File, type: 'image' | 'file') => {
    ...
    reader.onload = () => {
      sendMessage({ type, content: reader.result as string, fileName: file.name, fileSize: file.size });
      setShowAttach(false);
    };
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'var(--bg-1)', display: 'flex', flexDirection: 'column' }}>
        <header className="glass-strong" style={{
          ...
          <div style={{
            ...
            fontSize: '16px', fontWeight: 900, color: '#fff', position: 'relative', flexShrink: 0,
          }}>
            {active.avatar}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: '11px', color: isTyping ? 'var(--green-5)' : active.online ? 'var(--green-5)' : 'var(--text-3)' }}>
              {isTyping ? '✍️ يكتب...' : active.online ? '🟢 متصل الآن' : 'غير متصل'}
            </div>
          </div>
          ...
          {isTyping && (
            <div style={{ display: 'flex', alignSelf: 'flex-end' }}>
              <div style={{ padding: '12px 14px', background: 'var(--bg-3)', borderRadius: '18px 4px 18px 18px', border: '1px solid var(--border-2)', display: 'flex', gap: '4px' }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--text-3)', animation: 'pulse 1s ' + (i * 0.2) + 's ease-in-out infinite' }} />
                ))}
              </div>
            </div>
          )}
        ...
        {recording && (
          <div style={{ padding: '12px 16px', background: 'rgba(239,68,68,0.1)', borderTop: '1px solid rgba(239,68,68,0.3)', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '13px', color: '#F87171', fontWeight: 700 }}>🎤 جاري التسجيل... {fmtDuration(recordTime)}</span>
            <div style={{ flex: 1 }} />
        ...
        {!recording && (
          <div style={{
            padding: '10px 14px', paddingBottom: 'calc(10px + var(--safe-bottom))',
            background: 'var(--bg-glass-strong)', borderTop: '1px solid var(--border-2)',
            ...
            <button onClick={() => setShowAttach(!showAttach)} style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              flexShrink: 0, transform: showAttach ? 'rotate(45deg)' : 'none', transition: 'all 0.2s',
            }}>
              <Plus size={22} />
            </button>
            ...
            <div style={{ flex: 1, background: 'var(--bg-3)', border: '1px solid var(--border-2)', borderRadius: '22px', padding: '4px 12px' }}>
              <textarea
                ...
                style={{
                  ...
                  color: 'var(--text-0)', fontSize: '14px', resize: 'none', maxHeight: '100px',
                  direction: 'rtl', padding: '8px 6px', lineHeight: 1.5, outline: 'none',
                }}
              />
            </div>
            ...
            <button
              ...
              style={{
                ...
                background: input.trim() ? 'linear-gradient(135deg, var(--green-3), var(--green-5))' : 'var(--bg-3)',
        ...
        {showAttach && !recording && (
          <div style={{
            ...
            background: 'var(--bg-glass-strong)', border: '1px solid var(--border-3)',
            ...
            {[
              ...
              <button key={i} onClick={a.onClick} style={{
                ...
                background: 'var(--bg-3)', borderRadius: 'var(--r-md)', padding: '14px 8px',
      ...
      <div className="container-app" style={{ padding: '0 16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <div>
              ...
              <div style={{ fontSize: '11px', color: connected ? 'var(--green-5)' : 'var(--text-3)', marginTop: '2px' }}>
                {connected ? '🟢 متصل' : '⚠️ غير متصل بالسيرفر'} • {contacts.length} مستخدم
              </div>
            </div>
        ...
        {loadingContacts ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-3)' }}>جاري تحميل المستخدمين...</p>
          </div>
        ) : contacts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            <Users size={48} color="var(--text-4)" style={{ margin: '0 auto 16px' }} />
            <p style={{ fontSize: '15px', fontWeight: 700, marginBottom: '6px' }}>لا يوجد مستخدمون حالياً</p>
            <p style={{ fontSize: '12px', color: 'var(--text-3)' }}>
              عندما يسجّل أحد إخوانك في التطبيق، سيظهر هنا تلقائياً 🌙
            </p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {filtered.map((c, i) => (
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: '12px',
                  padding: '12px 8px', borderBottom: '1px solid var(--border-1)',
                  cursor: 'pointer', textAlign: 'right', borderRadius: 'var(--r-sm)', width: '100%',
                }}
              >
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  ...
                  {c.online && (
                    <div style={{
                      ...
                      background: 'var(--green-5)', border: '2px solid var(--bg-0)',
...
function Bubble({ msg, fromMe, showAvatar, active, fmtTime, fmtSize, fmtDuration }: any) {
  return (
    <div className="animate-fade-up" style={{
      display: 'flex', justifyContent: fromMe ? 'flex-start' : 'flex-end',
      alignItems: 'flex-end', gap: '8px', marginTop: showAvatar ? '8px' : '1px',
    }}>
      {!fromMe && (
      ...
      <div style={{
        ...
        background: fromMe ? 'linear-gradient(135deg, var(--green-3), var(--green-4))' : 'var(--bg-3)',
