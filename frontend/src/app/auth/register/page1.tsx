...
export default function RegisterPage() {
  ...
  const submit = async () => {
    setErr('');
    if (!data.name.trim()) return setErr('أدخل اسمك الكريم');
    ...
    try {
      ...
      const user = { name: data.name, email: data.email, points: 0, streak: 0, level: 'مبتدئ' };
  ...
  const guest = () => {
    localStorage.setItem('noor_user', JSON.stringify({ name: 'ضيف كريم', email: '', points: 0, streak: 0, level: 'مبتدئ' }));
    router.push('/home');
  };
    ...
    <div style={{
      ...
      <div className="container-app" style={{ padding: '24px' }}>
        ...
        <div className="glass-card animate-fade-up" style={{ padding: '24px' }}>
          <Field icon={<User size={18} />} placeholder="الاسم الكامل" value={data.name} onChange={v => setData({ ...data, name: v })} />
          ...
          <Field
            ...
            right={
              <button onClick={() => setShow(!show)} style={{ color: 'var(--text-3)' }}>
          ...
          <button onClick={guest} className="btn btn-ghost" style={{ width: '100%', padding: '13px' }}>
            👤 متابعة كضيف
          </button>
        ...
        <p style={{
          ...
          بإنشاء الحساب فإنك توافق على شروط الاستخدام<br />
